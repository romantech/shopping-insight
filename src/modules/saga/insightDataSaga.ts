...
function* getInsightData(action: GetInsightData) {
  try {
    // yield call : 결과 반환할 때까지 대기(동기 실행)
    // 첫번째 파라미터엔 Promise 반환 함수, 두번째 파라미터엔 해당 함수에 들어갈 인자
    // yield call(func, funcParams1, funcParams2)
    ...
    yield put(getDataSuccess(data)); // action dispatch
  } catch (err) {
    yield put(getDataFailed(err as Error));
  }
}
export default function* insightDataSaga(): Generator {
  ...
  // 첫번째 인자인 액션 객체가 들어오면 두번째 인자 함수 실행
}
